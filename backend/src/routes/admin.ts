...
export const adminRoute: FastifyPluginAsync = async (fastify) => {
    ...
    try {
      ...
      try {
        ...
        if (error.code === 9 || error.message?.includes('index')) {
          ...
          // Sort manually
          const docs = orgsSnapshot.docs.sort((a: admin.firestore.QueryDocumentSnapshot, b: admin.firestore.QueryDocumentSnapshot) => {
  ...
  fastify.post<{ Body: z.infer<typeof createOrgSchema> }>(
    ...
    async (request, reply) => {
      ...
      console.log(`📋 [ADMIN] Organization data:`, { orgId, ...orgData, createdAt: orgData.createdAt.toDate().toISOString() })
  ...
  fastify.post<{ Body: z.infer<typeof createInviteSchema> }>(
    ...
    async (request, reply) => {
      ...
      if (orgData.createdBy !== uid) {
        return reply.code(403).send({ error: 'You can only create invites for organizations you created' })
      }
      ...
      console.log(`✅ [ADMIN] Created invite: ${code} for org ${body.orgId} (${body.role}) by ${uid}`)
      ...
      const invitesPromises = orgIds.map(async (orgId) => {
        try {
          ...
          try {
            ...
            // If orderBy fails (missing index), try without orderBy
            if (error.code === 9 || error.message?.includes('index')) {
              console.warn(`⚠️ [ADMIN] Index missing for invites query (orgId: ${orgId}), fetching without orderBy`)
              invitesSnapshot = await db
                .collection('invites')
                .where('orgId', '==', orgId)
                .limit(limit)
  ...
  fastify.get<{ Params: { orgId: string } }>('/admin/orgs/:orgId/specialists', async (request, reply) => {
    try {
      await requireSuperAdmin(request, reply)
      ...
      // Check if organization belongs to this Super Admin
      if (orgData.createdBy !== uid) {
        return reply.code(403).send({ error: 'You can only view specialists for organizations you created' })
      }
      ...
      const specialists = await Promise.all(
        membersSnapshot.docs.map(async (doc) => {
          ...
          return {
            ...
            joinedAt: memberData.addedAt?.toDate?.()?.toISOString() || null,
            createdAt: specialistData?.createdAt?.toDate?.()?.toISOString() || null,
          }
        })
      )

      console.log(`✅ [ADMIN] Listed ${specialists.length} specialists for org ${orgId}`)
  ...
  fastify.get<{ Params: { orgId: string } }>('/admin/orgs/:orgId/parents', async (request, reply) => {
    try {
      await requireSuperAdmin(request, reply)
      ...
      // Check if organization belongs to this Super Admin
      if (orgData.createdBy !== uid) {
        return reply.code(403).send({ error: 'You can only view parents for organizations you created' })
      }

      // Get all parents linked to the organization
      const parentsSnapshot = await db
        .collection(`organizations/${orgId}/parents`)
        .get()

      const parents = parentsSnapshot.docs.map((doc: admin.firestore.QueryDocumentSnapshot) => ({
        id: doc.id,
        ...doc.data(),
  ...
  fastify.get<{ Params: { orgId: string } }>('/admin/orgs/:orgId/children', async (request, reply) => {
    try {
      await requireSuperAdmin(request, reply)
      ...
      // Check if organization belongs to this Super Admin
      if (orgData.createdBy !== uid) {
        return reply.code(403).send({ error: 'You can only view children for organizations you created' })
      }

      // Get all children assigned to the organization
      const childrenSnapshot = await db
        .collection(`organizations/${orgId}/children`)
        .where('assigned', '==', true)
        .get()

      const childIds = childrenSnapshot.docs.map((doc: admin.firestore.QueryDocumentSnapshot) => doc.id)
        ...
        childIds.map(async (childId) => {
          ...
          if (!childSnap.exists) {
            return {
              ...
              name: 'Unknown',
              age: undefined,
              assignedAt: childrenSnapshot.docs.find(d => d.id === childId)?.data()?.assignedAt?.toDate?.()?.toISOString() || null,
            }
          }
